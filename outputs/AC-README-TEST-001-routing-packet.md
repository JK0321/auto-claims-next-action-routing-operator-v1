# Auto Claims Next-Action Routing Packet

**Generated Output Type:** Routing Packet

---

## 1. Claim Snapshot

| Field | Value |
|---|---|
| **Case ID** | AC-README-TEST-001 |
| **Claim Type** | Personal Auto Property Damage |
| **Date of Loss** | 2026-06-01 |
| **Loss Location** | Fort Collins, CO |
| **Insured Name** | Michael Turner |
| **Claimant Name** | Rachel Adams |
| **Coverage Status** | Confirmed active for date of loss |
| **Liability Status** | Clear — insured rear-ended claimant at stoplight |

---

## 2. Loss Summary

Insured Michael Turner rear-ended claimant Rachel Adams at a stoplight in Fort Collins, Colorado on 2026-06-01. Coverage is confirmed active for the date of loss. No injury, attorney involvement, or SIU indicators are present. The primary remaining work is obtaining a damage estimate for minor rear bumper damage.

---

## 3. Routing Decision

| Field | Value |
|---|---|
| **Primary Route** | MATERIAL DAMAGE / APPRAISAL UNIT |
| **Routing Confidence** | High |

Coverage is confirmed active for the date of loss. Liability is uncontested — clear rear-end impact at a stoplight, no dispute noted. No injury claim, attorney representation, SIU indicators, or field inspection need is present. Missing police report and pending damage estimate are both non-blocking for routing to the material damage / appraisal unit.

---

## 4. Decision Hierarchy Applied

Coverage is confirmed and no coverage question is present — Decision 3 does not apply. No SIU indicators are noted — Decision 4 does not apply. No injury is claimed and no attorney representation is present — Decision 5 does not apply. The primary remaining work is obtaining a damage estimate for minor rear bumper damage, with vehicle photos already in file — Decision 6 (MATERIAL DAMAGE / APPRAISAL UNIT) applies. Decision 8 (STANDARD DESK AUTO UNIT) is not reached because appraisal coordination is still outstanding.

---

## 5. Sourcing, Governance, Capacity, and Modifier Flags

| Field | Value |
|---|---|
| **Sourcing Model** | INTERNAL DESK |
| **Governance Action** | DIRECT ASSIGNMENT |
| **Assignment Resource** | DESK-AUTO-01 |
| **Workload Status** | 13/15 — Available |
| **Capacity Action** | Direct assignment allowed. |
| **Source Documents Reviewed** | Intake referral — AC-README-TEST-001 — all required routing facts provided; coverage, liability, injury, and SIU status all explicitly addressed. |

**Modifier Flags:**

- POLICE REPORT MISSING / UNAVAILABLE

---

## 6. Missing Information / Constraints

- Policy number / reference not provided. Non-blocking: coverage is confirmed active for the date of loss and no coverage question is present.
- Damage estimate not yet received. Non-blocking: appraisal coordination is the purpose of this routing step.
- Police report not available. Non-blocking for routing; receiving unit should confirm whether it is required for the appraisal workflow.

---

## 7. Next Action

Assign to DESK-AUTO-01 for appraisal coordination. Use available vehicle photos to initiate desk appraisal review. Request damage estimate from claimant's repair shop or coordinate direct inspection as appropriate. Confirm whether police report is required for the appraisal workflow before closing the routing review.

---

## 8. Draft Routing Note

**TO:** Material Damage / Appraisal Unit — Internal Desk (DESK-AUTO-01)

**RE:** AC-README-TEST-001 — Direct Assignment

Clear rear-end loss. Insured Michael Turner rear-ended claimant Rachel Adams at a stoplight in Fort Collins, CO on 2026-06-01. Coverage confirmed active for date of loss. No injuries, no attorney, no SIU indicators. Minor rear bumper damage; vehicle photos in file; estimate not yet received. Police report unavailable — confirm whether required for the appraisal workflow. Workload confirmed within target. Direct assignment authorized.

---

*This routing packet was produced by the Auto Claims Next-Action Routing Operator. It is a routing recommendation only. Real assignment must happen in your carrier, TPA, vendor, or claim-management system.*
